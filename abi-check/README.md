...
**Why a linker is not enough.** C has no name mangling. If a header says
...
the panel fills with noise. Nothing in either toolchain will tell you. This is
what tells you: it compares parameter *types and order*, not just presence —
which is the half the gate's `symbols_agree` cannot see, and the half a link
error never reaches.

**Who uses it.** Five boundaries across two repositories, each checked where
it is owned:
...
they are. The third row above is the same header against the *Rust* host
doubles, which is a different guarantee: it keeps the doubles honest, not the
shim. The last row is why `xpui-fui` exports its macro's source as a string: a
git dependency lands in a cargo checkout directory with no path a sibling
repository can read.
